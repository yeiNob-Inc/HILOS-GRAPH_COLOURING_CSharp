...
void removeElAtPos(vector<int> &A, int x){
	//Removes the element at position x (we can oly use this when the element ordering is unimportant)
	A[x] = A.back();
	A.pop_back();
}
inline
...
int rouletteWheel(vector<double> &tauEta, double tauEtaTotal){
	if(tauEta.size()==0) {cout<<"Error: Roulette wheel with no choices"<<endl; exit(1);}
...
//-------------------------------------------------------------------------------------
bool buildSolution(Graph &g, vector<vector<int> > &sol, vector<vector<int> > &neighbours, vector<int> &degree, vector<vector<double> > &t, int k, double alpha, double beta, 
	int &colsUsed, int numISets, vector<int> &X, vector<int> &Y, vector< vector<int> > &tempX, vector< vector<int> > &tempY, vector< vector<int> > &ISet, vector<double> &tauEta)
{
	...
	while(numColoured < g.n){
		...
		for(iset=0;iset<numISets;iset++){
			...
			//Choose the first node at random from tempY[iset]. Add to ISet[iset]. "Remove" from tempX[iset] and 
			//remove all adjacents from tempY[iset]
			...
			//Choose the remaining nodes due to a) pheremone (measured by tau) and degrees of nodes in the 
			//current subgraph induced by tempX[iset]			
